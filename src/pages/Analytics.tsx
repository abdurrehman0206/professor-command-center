...
const Analytics: React.FC = () => {
	...
	const gradeDistribution = [
		...
		},
		{
			...
			count: gradedSubmissions.filter(
				(sub) => (sub.grade || 0) >= 80 && (sub.grade || 0) < 90,
			).length,
			color: "bg-blue-500",
		},
		{
			grade: "C (70-79)",
			count: gradedSubmissions.filter(
				(sub) => (sub.grade || 0) >= 70 && (sub.grade || 0) < 80,
			).length,
			color: "bg-yellow-500",
		},
		{
			grade: "D (60-69)",
			count: gradedSubmissions.filter(
				(sub) => (sub.grade || 0) >= 60 && (sub.grade || 0) < 70,
			).length,
			color: "bg-orange-500",
		},
		{
	...
	const assignmentCompletion = assignments.map((assignment) => {
		const assignmentSubmissions = courseSubmissions.filter(
	...
	const maxCompletionRate = Math.max(
		...assignmentCompletion.map((a) => a.completion),
	);
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									...
									<Badge variant="outline" className="text-xs">
										{totalPossibleSubmissions > 0
											? Math.round(
													(onTimeSubmissions / totalPossibleSubmissions) * 100,
												)
											: 0}
										%
									</Badge>
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									...
									<Badge variant="outline" className="text-xs">
										{totalPossibleSubmissions > 0
											? Math.round(
													(lateSubmissions / totalPossibleSubmissions) * 100,
												)
											: 0}
										%
									</Badge>
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									...
									<Badge variant="outline" className="text-xs">
										{totalPossibleSubmissions > 0
											? Math.round(
													(missingSubmissions / totalPossibleSubmissions) * 100,
												)
											: 0}
										%
									</Badge>
